...
//    }
    ...
    public Map<Date, List<Item>> syncCalendars(List<Person> users) {
        return null;
//        Map<Date, List<Item>> busyItemMap = new HashMap<Date, List<Item>>();
//        ObjectContainer db = null;
//        try {
//            db = DBUtil.getDB();
//            DB4OReceivedItemsNotDoneQuery query = new DB4OReceivedItemsNotDoneQuery();
//            for (Person user : users) {
//                List<Item> items = query.run(user,archiveIncluded db);
//                for (Item item : items) {
//                    List<Item> busyItems = busyItemMap.get(item.getDeadLine());
//                    if (busyItems == null) {
//                        busyItems = new ArrayList<Item>();
//                        busyItemMap.put(item.getDeadLine(), busyItems);
//                    }
//                    busyItems.add(item);
//                }
//            }
