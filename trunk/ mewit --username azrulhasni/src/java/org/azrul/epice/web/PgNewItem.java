...
public class PgNewItem extends AbstractPageBean {
    ...
    public void init() {
        ...
        if (("PgReceivedItem").equals(getSessionBean1().getPreviousPage())) {
            ...
            //if we came back from viewing attachments
            ...
            //if delegate, set current supervisors visible
    ...
    public String btnSubmit_action() {
        ...
        if (newItems != null) {
            ...
            if (newItems.iterator().hasNext()) {
                ...
                //deal with if we are delegating
                ...
                //reset current file repository
                sessionBean1.setCurrentFileRepository(newItems.iterator().next().getFileRepository());

                //save to users as buddies
                //get to-users from one of the new items created
            ...
            //reestablish file repo from the created items
            sessionBean1.setCurrentFileRepository(newItems.iterator().next().getFileRepository());

            //also, see if any of the items is sent to self
    ...
    public String btnGotoAttachment_action() {
        ...
        //if no file repository
        if (sessionBean1.getCurrentFileRepository() == null) {
            //but there is a file repo attached to the parent task in the DB
            //then, use it
            int state = 0;
            if (getSessionBean1().getParent() == null) { //not a delegation
            ...
            if (state == 1) { // not a delegation (original new item) or delegating a task without file repo => attach an empty file repo
        ...
        if (!newItems.isEmpty()) {
            //take one item and save it to "edited item"
    ...
    private void setupUIFromItem(Item item) {
        ...
        for (Person toUser : item.getToUsers()) {
            //getSessionBean1().getToUsersEmail().put(toUser.getEmail(), toUser.getEmail() + "*");
        ...
        //stSupervisors is handled in init
