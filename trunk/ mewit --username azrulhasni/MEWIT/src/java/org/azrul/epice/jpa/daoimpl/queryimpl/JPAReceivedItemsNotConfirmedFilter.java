 ...
 */
public class JPAReceivedItemsNotConfirmedFilter implements JPAItemsFilter {
    ...
    public JPAReceivedItemsNotConfirmedFilter (){
        type = "RECEIVED_ITEMS_NOT_CONFIRMED";
    }
    
    public boolean filter(Item item, String user) {
       return item.getDeadLine() != null && item.getRecipient().equals(user) && item.getStatus().equals(Item.Status.UNCONFIRMED);
    }
