 ...
 */
public class DB4OSentItemsDoneUnconfirmedFilter implements ItemsFilter {
    ...
    public boolean filter(Item item, Person user) {
        return (item.getDeadLine() != null && item.getFromUser().equals(user) && item.getStatus().equals("DONE-UNCONFIRMED"));  
    }
}
